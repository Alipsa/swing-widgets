...
`usesDerivedPattern` is `false` only when the 5-arg constructor receives a non-null `datePattern` argument. All other constructors set it to `true`. This lets the picker distinguish "the caller explicitly chose `yyyy-MM-dd`" from "we happened to derive `yyyy-MM-dd` from the locale" even though both cases produce the same resolved string.
...
5. super.setLocale(newLocale)           // keep Swing component locale in sync
6. this.locale = newLocale
7. if usesDerivedPattern → this.datePattern = derivePattern(newLocale)
8. rebuild textField:
...
9. layoutComponents()                   // removeAll() + re-add with current TextFieldPosition
```
...
`CalendarPanel` needs no changes. `openPopup()` already constructs it fresh each time, so it picks up the updated `locale` automatically. `datePattern` is only used by the text field; `CalendarPanel` does not use it.
...
| `new DatePicker(from, to, initial, locale, "yyyy-MM-dd")` (5-arg) | `false` | pattern unchanged |
